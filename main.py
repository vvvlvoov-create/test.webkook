...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_data['type'] == 'rr':
        ...
        await update_rr_message(context.bot)
    else:  # pd
        ...
        # Обновляем сообщение в канале
        await update_pd_message(context.bot)
    
    # Очищаем состояние пользователя
    del user_states[user_id]

async def update_rr_message(bot: Bot):
...
async def update_pd_message(bot: Bot):
...
async def send_initial_message(bot: Bot):
    ...
    try:
        ...
        if time(0, 0) <= now <= time(5, 0):
            ...
            message = await bot.send_message(CHAT_ID, rr_text, reply_markup=keyboard)
            current_rr_message = {
                'chat_id': CHAT_ID,
                'message_id': message.message_id
            }
            logging.info("✅ Отправлено начальное RR сообщение")
            ...
            message = await bot.send_message(CHAT_ID, pd_text, reply_markup=keyboard)
            current_pd_message = {
                'chat_id': CHAT_ID,
                'message_id': message.message_id
            }
            logging.info("✅ Отправлено начальное PD сообщение")
            
    except Exception as e:
        logging.error(f"Ошибка при отправке начального сообщения: {e}")

async def auto_post_messages(application: Application):
    ...
    try:
        ...
        if current_time.hour == 0 and current_time.minute == 0:
            ...
            message = await application.bot.send_message(CHAT_ID, rr_text, reply_markup=keyboard)
            current_rr_message = {
                'chat_id': CHAT_ID,
                'message_id': message.message_id
            }
        ...
        # В 5:01 отправляем PD лист  
            ...
            # Отправляем новое сообщение
            message = await application.bot.send_message(CHAT_ID, pd_text, reply_markup=keyboard)
            current_pd_message = {
                'chat_id': CHAT_ID,
                'message_id': message.message_id
            }
...
def main():
    ...
    async def post_init(application: Application):
        ...
        await send_initial_message(application.bot)
        logging.info("✅ Начальное сообщение отправлено")
    
    # Привязываем post_init к приложению
    application.post_init = post_init
    
    # Добавляем job_queue для автоматических сообщений
    job_queue = application.job_queue
    if job_queue:
        job_queue.run_repeating(auto_post_messages, interval=60, first=10)
        logging.info("✅ Job queue запущен для автоматических сообщений")
